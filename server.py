...
class P2P:
    ...
    def create_session(self, _address: str):
        ...
        if ind is None:
            self.log.save_data(
                "All sockets are busy, can`t connect to {}".format(_address)
            )
            return
    ...
    def send(self, _address: str, _message: str):
        ...
        try:
            ...
            self.client_sockets[ind].send(
                rsa.encrypt(_message.encode(), self.keys[ind])
            )
    ...
    def __add_keys(
        self, _address: str, _key: rsa.key.PublicKey, _my_key: rsa.key.PrivateKey
    ):
        ind = self.__get_ind_by_address(_address)
        try:
            self.keys[ind] = _key
    ...
    def __add_request(self, _address: str, _message: bytes):
        ...
        self.clients_logs[self.__get_ind_by_address(_address)].save_data(
            "from {}: {}".format(_address, str(_message))
        )
    ...
    def get_request(self, _address: str):
        ...
        self.incoming_requests[_address] = [
            self.incoming_requests[_address][i]
